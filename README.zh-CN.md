...
**2026-09-11 更新：**无障碍树定位、事件驱动的页面观察，以及 X／小红书只读工具。
具体改动、升级方法和验证范围见[更新日志](CHANGELOG.md)。
...
现在底层会结合 Chrome 的无障碍树和 DOM 布局信息：先取得按钮的可访问名称和状态，再让 ref 指向浏览器里的真实节点 ID。不会再往网页元素上添加临时属性；同名按钮仍分别对应自己的节点，也支持开放 Shadow DOM 里的控件。快照目前描述主文档，不展开子框架内部的控件。
...
现在默认共 14 个 MCP 工具，另有 2 个需要显式配置才会出现的“一起逛”读取工具。

## 这次新增的社交阅读

人先在专用浏览器里登录，再调用 `social_feed` 读取 X 或小红书的信息流；传入 `query` 就是搜索。拿到结果后，把完整帖子 URL 和 `tabRef` 交给 `social_read`，就能继续读正文和已经加载出来的讨论。

每个平台会复用自己的标签页，最近列表保留三分钟。列表里的帖子会按准确链接点击，读完在安全时返回列表；其他完整链接直接打开。不会随便点第一张卡片，也不会关闭无关标签页。小红书链接里的 `xsec_token` 要保留，短链接暂不支持。

一次最多读取 30 张列表卡片，详情最多带回 40 条已加载的讨论。文字和卡片可读就继续，不必等图片、字体和脚本全部加载，也不用每一步都截图再问模型。滚动次数和等待时间都有上限。

这两个工具只负责阅读，不提供点赞、评论和发帖。遇到登录、验证、敏感表单、目标消失或页面被人切走，会停下来。X 的 `thread` 是可见会话内容，可能包含上文或推荐项，不保证都是直接回复；小红书也只读取已经渲染的评论，不代表完整评论区。站点结构变化后可能需要更新适配，这些改进不保证免于平台风控。

选择器和列表／详情流程参考了 [blueberriely/ai-social-browser](https://github.com/blueberriely/ai-social-browser)，保留其[独立 MIT 许可和署名](src/third-party/ai-social-browser.LICENSE)。

“一起逛”的内部监听也换成了 `MutationObserver` 配合导航、焦点事件：没变化时不再每秒扫描标题和焦点，有变化再安排读取。仍保留 5 秒页面预览、15 秒正文窗口和文字去重，持续变化也不会无限推迟正文。关闭开关后会断开监听。它不会自动让 MCP 客户端常驻，也不改变图片下载或 noVNC 的画面传输速度。
